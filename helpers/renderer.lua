...
do
	...
	function WindowRenderTarget.New(renderer)
		...
		self.depth_image_view = renderer.device:CreateImageView(
			self.depth_image.ptr[0],
			{format = depth_format, aspect = "depth"}
		)
		-- Create render pass for swapchain format with depth
		self.render_pass = renderer.device:CreateRenderPass(
			renderer.surface_formats[renderer.config.surface_format_index],
			nil,
			nil,
			depth_format
		)
	...
	function WindowRenderTarget:RecreateSwapchain()
		...
		self.depth_image_view = self.renderer.device:CreateImageView(
			self.depth_image.ptr[0],
			{format = depth_format, aspect = "depth"}
		)
