...
function output.Initialize()
	...
	output.original_stdout_fd = assert(fs.fd.stdout:dup())
	output.original_stdout_file = assert(fs.file_open(output.original_stdout_fd.fd, "w"))
	local log_fd_num = output.file:get_fileno()
	...
	process_pipe = function()
		while true do
			local data, n = read_fd:read(4096 - 1)
...
function output.Shutdown()
	...
	assert(output.original_stdout_fd:dup(fs.fd.stdout))
	assert(output.original_stdout_fd:close())
