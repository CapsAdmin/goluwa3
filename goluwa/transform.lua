...
function META:GetCameraDistance()
	local p = render3d.GetCameraPosition()
	local x = self.TRMatrix.m30 - -p.y
	local y = self.TRMatrix.m31 - -p.x
	local z = self.TRMatrix.m32 - -p.z
	return x * x + y * y + z * z
end
...
function META.New()
	local obj = prototype.CreateObject("transform")
	obj:Initialize()
	return obj
end
