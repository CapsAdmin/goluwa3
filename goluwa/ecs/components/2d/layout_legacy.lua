...
function META:Initialize()
	...
	self.Owner:AddLocalListener("OnTransformChanged", function()
		-- Only invalidate parent layout if SIZE changed, not just position
		-- This prevents unnecessary parent re-layouts when child is merely positioned
		local should_invalidate = false

		if self.Owner and self.Owner.transform then
			local old_size = self:GetLayoutSize()
			local new_size = self.Owner.transform:GetSize()

			-- Invalidate if we have a cached size and it differs from current
			if old_size and not (old_size == new_size) then
				should_invalidate = true
			end

			-- Also invalidate if we don't have a cached size yet (first time)
			if not old_size then should_invalidate = true end

			-- Update LayoutSize AFTER checking but BEFORE invalidating
			-- This way it's available for the next check
			if not layout_lib.IsRunning() and not self:IsBusy() then
				self:SetLayoutSize(new_size:Copy())
			end
		end

		if should_invalidate then self:InvalidateLayout() end
	end)
...
function META:CalcLayoutInternal()
	...
	-- Update LayoutSize after layout completes to match the new size
	if tr_comp then self:SetLayoutSize(tr_comp.Size:Copy()) end
-- Note: We no longer invalidate here based on size/position changes
-- The OnTransformChanged handler now properly detects SIZE changes and invalidates
-- This prevents over-invalidation when layout just positions children
end
