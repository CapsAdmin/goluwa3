...
local function draw_text_entry(entry)
	local screen_pos = render3d.GetCamera():WorldPositionToScreenUnjittered(entry.position)
...
local function draw_2d_line_entry(entry)
	local from, to = render3d.GetCamera():WorldLineToScreenUnjittered(entry.from, entry.to)
