...
local function build_capsule_triangle_contact(collider, hit, v0, v1, v2, options)
	...
	if not (shape and shape.GetRadius and shape.GetBottomSphereCenterLocal and shape.GetTopSphereCenterLocal) then return nil end
	...
	if distance > epsilon then
		...
		normal = center_delta:GetLength() > epsilon and (center_delta / center_delta:GetLength()) or triangle_normal
	end
...
local function collect_sphere_swept_triangle_contacts(body, collider, kind, contacts, options, owner, filter, cast_up)
	...
	if not (hit and hit.model and hit.distance <= movement_length + cast_up) then return false end
		...
		if depth and depth > epsilon then
			finalize_world_contact(
				...
				contacts,
				{
					...
					normal = normal,
					depth = depth,
					feature_key = triangle_local_feature_key and triangle_local_feature_key(hit, local_point, normal) or nil,
				},
				local_point
			)
		end
...
local function collect_capsule_swept_triangle_contacts(body, collider, kind, contacts, options, owner, filter, cast_up)
	...
	if not (shape and shape.GetRadius and shape.GetBottomSphereCenterLocal and shape.GetTopSphereCenterLocal) then return false end
...
function world_contact_sampling.CollectSweepContacts(body, kind, contacts, options)
	...
	for _, collider in ipairs(body:GetColliders() or {}) do
		...
		if allow_sweep_sampling then
			if collect_polyhedron_swept_triangle_contacts(body, collider, kind, contacts, options, owner, filter, cast_up) then
				goto continue_collider
			end

			if shape_type == "capsule" and collect_capsule_swept_triangle_contacts(body, collider, kind, contacts, options, owner, filter, cast_up) then
				goto continue_collider
			end
			...
			for _, sample in ipairs(world_contact_sampling.BuildColliderSamples(body, collider, sample_source, local_point_key)) do
				...
				if sweep_distance > epsilon then
					local hit = physics.Sweep(
						previous_point,
						sample.preferred_direction and (sample.preferred_direction * (sweep_distance + cast_up)) or sweep,
						0,
						owner,
						filter
					)
