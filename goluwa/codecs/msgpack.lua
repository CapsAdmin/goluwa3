...
if LITTLE_ENDIAN then
	...
	sbuffer_append_int64 = function(self, n, h)
		...
		for i = 24, 8, -8 do
			t[#t + 1] = band(rshift(q, i), 0xff)
		end

		t[5] = band(q, 0xff)

		for i = 24, 8, -8 do
			t[#t + 1] = band(rshift(r, i), 0xff)
		end
...
local ljp_unpack_raw = function(ptr, size, offset)
	...
	local buf = {data = ffi.cast("unsigned char *", ptr), size = size}
	local data
