...
local function create_font(path, size, glyphs, texture)
	local self = line.CreateObject("Font", love)
...
do
	local Text = line.TypeTemplate("Text", love)
	...
	function love.graphics.newText(font, text)
		...
		local self = line.CreateObject("Text", love)
		self.font = font
		self:set(text or "")
		return self
	end

	line.RegisterType(Text, love)
end

line.RegisterType(Font, love)
