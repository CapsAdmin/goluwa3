...
local function lasterror(num)
	if ffi.os == "Windows" then
		...
		if err and err:sub(-2) == "\r\n" then return err:sub(0, -3), code end
...
if ffi.os == "Windows" then
	...
	function process.setenv(key, val)
		local res = ffi.C.SetEnvironmentVariableA(key, val)
	...
	function process.spawn(opts)
		...
		if opts.stdin == "pipe" then
			...
			if ffi.C.CreatePipe(hr, hw, sa, 0) == 0 then return nil, lasterror() end
	...
	function meta:wait()
		...
		if result ~= WAIT_OBJECT_0 then return nil, lasterror() end
	...
	function meta:write(data)
		if not self.stdin then return nil, "stdin is not piped" end
	...
	function meta:read(size)
		if not self.stdout then return nil, "stdout is not piped" end
	...
	function meta:read_err(size)
		if not self.stderr then return nil, "stderr is not piped" end
	...
	local function set_nonblocking(fd)
		...
		if flags < 0 then return nil, lasterror() end
	...
	function process.setenv(key, val)
		local res = ffi.C.setenv(key, val, 1)
