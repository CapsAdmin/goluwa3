...
do
	...
	do
		...
		table.insert(deferred_type_assignments, function()
			types[0] = ffi.cast('const struct wl_interface*', interface_ptrs['zwlr_data_control_source_v1'])
		end)
	...
	do
		...
		table.insert(deferred_type_assignments, function()
			types[0] = ffi.cast('const struct wl_interface*', interface_ptrs['zwlr_data_control_device_v1'])
		end)
	...
	local name_str = ffi.new('char[?]', #'zwlr_data_control_manager_v1' + 1)
	ffi.copy(name_str, 'zwlr_data_control_manager_v1')
	...
	interfaces['zwlr_data_control_manager_v1'] = iface_ptr[0]
	interface_ptrs['zwlr_data_control_manager_v1'] = iface_ptr
	interface_data['zwlr_data_control_manager_v1'] = data
end
...
do
	...
	do
		...
		local types = ffi.new('const struct wl_interface*[1]')
		table.insert(deferred_type_assignments, function()
			types[0] = ffi.cast('const struct wl_interface*', interface_ptrs['zwlr_data_control_offer_v1'])
		end)
	...
	local name_str = ffi.new('char[?]', #'zwlr_data_control_device_v1' + 1)
	ffi.copy(name_str, 'zwlr_data_control_device_v1')
	local iface_ptr = ffi.new('struct wl_interface[1]')
	iface_ptr[0].name = name_str
	iface_ptr[0].version = 2
	iface_ptr[0].method_count = 3
	...
	data.name_str = name_str
	data.iface_ptr = iface_ptr
	interfaces['zwlr_data_control_device_v1'] = iface_ptr[0]
	interface_ptrs['zwlr_data_control_device_v1'] = iface_ptr
	interface_data['zwlr_data_control_device_v1'] = data
end
...
do
	...
	local name_str = ffi.new('char[?]', #'zwlr_data_control_source_v1' + 1)
	ffi.copy(name_str, 'zwlr_data_control_source_v1')
	local iface_ptr = ffi.new('struct wl_interface[1]')
	iface_ptr[0].name = name_str
	...
	data.name_str = name_str
	data.iface_ptr = iface_ptr
	interfaces['zwlr_data_control_source_v1'] = iface_ptr[0]
	interface_ptrs['zwlr_data_control_source_v1'] = iface_ptr
	interface_data['zwlr_data_control_source_v1'] = data
end
...
do
	...
	local name_str = ffi.new('char[?]', #'zwlr_data_control_offer_v1' + 1)
	ffi.copy(name_str, 'zwlr_data_control_offer_v1')
	local iface_ptr = ffi.new('struct wl_interface[1]')
	iface_ptr[0].name = name_str
	iface_ptr[0].version = 1
	iface_ptr[0].method_count = 2
	...
	data.name_str = name_str
	data.iface_ptr = iface_ptr
	interfaces['zwlr_data_control_offer_v1'] = iface_ptr[0]
	interface_ptrs['zwlr_data_control_offer_v1'] = iface_ptr
...
do
		...
		for i, evt in ipairs(iface.events) do
			...
			if cb then
				...
				local cb_func = ffi.cast(sig, function(data, proxy, ...)
					...
					proxy = ffi.cast('struct zwlr_data_control_manager_v1*', proxy)
	...
	output_table['zwlr_data_control_manager_v1'] = meta
...
do
		...
		for i, evt in ipairs(iface.events) do
			...
			if cb then
				...
				local cb_func = ffi.cast(sig, function(data, proxy, ...)
					...
					proxy = ffi.cast('struct zwlr_data_control_device_v1*', proxy)
	...
	output_table['zwlr_data_control_device_v1'] = meta
...
do
		...
		for i, evt in ipairs(iface.events) do
			...
			if cb then
				...
				local cb_func = ffi.cast(sig, function(data, proxy, ...)
					...
					proxy = ffi.cast('struct zwlr_data_control_source_v1*', proxy)
	...
	output_table['zwlr_data_control_source_v1'] = meta
...
do
		...
		for i, evt in ipairs(iface.events) do
			...
			if cb then
				...
				local cb_func = ffi.cast(sig, function(data, proxy, ...)
					...
					proxy = ffi.cast('struct zwlr_data_control_offer_v1*', proxy)
	...
	output_table['zwlr_data_control_offer_v1'] = meta
	ffi.metatype('struct zwlr_data_control_offer_v1', meta)
end
