...
do -- event extension
	...
	if SERVER then
		function event.CallOnClient(event_type, filter, ...)
			message.Send("evtmsg", filter, event_type, ...)
		end

		function event.BroadcastCall(event_type, ...)
			event.CallOnClient(event_type, nil, ...)
		end

		function event.CallShared(event_type, ...)
			event.Call(event_type, ...)
			event.BroadcastCall(event_type, ...)
		end
	end
end
