...
function IRCClient:OnMessage(message, nick, ip) end

function IRCClient:OnReady() end

function IRCClient:OnJoin(nick) end

function IRCClient:OnPart(nick) end

function IRCClient:OnNickChanged(nick, new) end

function IRCClient:Query(cmd, response, terminator, callback)
	self:Send(cmd)
	list.insert(
		self.queries,
...
function IRCClient:SetNick(str)
	self.Nick = str
	self:NICK(str)
end

function IRCClient:Join(channel)
	self:JOIN(channel)
end

function IRCClient:Connect(address, port)
...
function IRCClient:OnRemove()
	self:QUIT(":object removed")
	self.socket:Remove()
end

function IRCClient.New()
	local self = IRCClient:CreateObject()
	self.queries = {}
	return self
end

return IRCClient:Register()
