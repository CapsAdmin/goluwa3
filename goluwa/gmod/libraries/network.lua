...
local packet = rawget(_G, "packet")
local engine_network = rawget(_G, "network")

local function get_hostname()
	if engine_network and engine_network.GetHostname then
		return engine_network.GetHostname() or "no hostname!"
	end
...
do
	...
	function META:GetInfo(key)
		return gine.env.GetConVarString(key)
	end
end
