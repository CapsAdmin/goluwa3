...
scene_voxelizer.DEFAULT_BASE_VOXEL_SIZE = 0.33
...
local function build_grid_world_aabb(clipmap, origin_override)
...
local function create_volume_target(owner_grid, clipmap, axis_name, group_config)
...
function grid:EnsureClipmap(index)
...
function grid:EnsureResources(index)
...
function grid:SetOrigin(index, origin)
	local clipmap = self:EnsureClipmap(index)
	clipmap.origin = copy_vec3(origin)
	clipmap.world_aabb = build_grid_world_aabb(clipmap, clipmap.origin)
	return clipmap.origin
end

function grid:SetBuildOrigin(index, origin)
	local clipmap = self:EnsureClipmap(index)
	clipmap.build_origin = copy_vec3(origin)
	clipmap.build_world_aabb = build_grid_world_aabb(clipmap, clipmap.build_origin)
...
function grid:WorldToVoxel(index, world_position)
...
function grid:VoxelToWorld(index, voxel_position)
	local clipmap = self:GetClipmap(index)
