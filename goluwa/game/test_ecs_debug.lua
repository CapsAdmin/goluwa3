...
create_box("X_axis", Vec3(bar_length/2 + 2, 0, 0), Vec3(bar_length, bar_thickness, bar_thickness), red_material)

-- Y axis = GREEN bar (pointing in +Y direction, which is "left" in Source)
create_box("Y_axis", Vec3(0, bar_length/2 + 2, 0), Vec3(bar_thickness, bar_length, bar_thickness), green_material)

-- Z axis = BLUE bar (pointing in +Z direction, which is "up" in Source)
create_box("Z_axis", Vec3(0, 0, bar_length/2 + 2), Vec3(bar_thickness, bar_thickness, bar_length), blue_material)
...
local function create_arrow(name, position, angles, material)
	...
	if angles then
		entity.transform:SetAngles(angles)
	end
...
create_arrow("arrow_yaw_90", rot_offset + Vec3(0, 0, 5), Deg3(0, 90, 0), Material.New({ base_color_factor = {0, 1, 1, 1} }))

-- Yaw -90° (rotate around Z axis) - should now point along world -Y direction
create_arrow("arrow_yaw_-90", rot_offset + Vec3(0, 0, 10), Deg3(0, -90, 0), Material.New({ base_color_factor = {1, 0, 1, 1} }))

-- Pitch +90° (nose down) - should now point along world -Z = DOWN
create_arrow("arrow_pitch_90", rot_offset + Vec3(0, 0, 15), Deg3(90, 0, 0), Material.New({ base_color_factor = {1, 0.5, 0, 1} }))

-- Pitch -90° (nose up) - should now point along world +Z = UP  
create_arrow("arrow_pitch_-90", rot_offset + Vec3(0, 0, 20), Deg3(-90, 0, 0), Material.New({ base_color_factor = {0.5, 0, 1, 1} }))

-- Roll +90° - should spin around X axis (long axis of arrow)
create_arrow("arrow_roll_90", rot_offset + Vec3(0, 0, 25), Deg3(0, 0, 90), Material.New({ base_color_factor = {0.5, 1, 0.5, 1} }))
...
-- Draw
event.AddListener("Draw3D", "test_ecs_debug", function(cmd, dt)
