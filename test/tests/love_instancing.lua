...
T.Test2D("love graphics drawInstanced respects parent transforms", function()
	local love = new_love_graphics_env("11.0.0")
	local image = make_quadrant_image(love)
	local mesh = love.graphics.newMesh(
		{
	...
	love.graphics.setColor(1, 1, 1, 1)
	love.graphics.push()
	love.graphics.translate(48, 24)
	love.graphics.setShader(shader)
	love.graphics.drawInstanced(mesh, 1)
	love.graphics.setShader()
	love.graphics.pop()
	return function()
		T.AssertScreenPixel{pos = {88, 64}, color = {1, 1, 0, 1}, tolerance = 0.1}
		T.AssertScreenPixel{pos = {40, 40}, color = {0, 0, 0, 1}, tolerance = 0.1}
	end
end)
