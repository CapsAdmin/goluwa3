...
T.Test("panel constructor 2d entities", function()
	local pnl = Panel({Name = "test_panel"})
	T(pnl:IsValid())["=="](true)
	T(pnl.transform)["~="](nil)
	T(pnl.rect)["~="](nil)
	T(pnl.mouse_input)["~="](nil)
	local txt = Text({Name = "test_text", Parent = pnl})
	T(txt:IsValid())["=="](true)
	T(txt.text)["~="](nil)
	T(txt.transform)["~="](nil)
	T(txt:GetParent())["=="](pnl)
...
T.Test("panel mouse input states", function()
	local pnl = Panel({Name = "mouse_test"})
...
T.Test("panel animations basic", function()
	local pnl = Panel({Name = "anim_test"})
...
T.Test("panel mouse simulation and hover", function()
	...
	local pnl = Panel({Name = "sim_target", Parent = world})
