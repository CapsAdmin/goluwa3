...
T.Test2D("Graphics render2d preserves fragment constant values across resolved storage", function()
	render2d.SetUV2(0.25, 0.5, 0.75, 1.0)
	render2d.SetBlur(3, 5)
	...
	T(shape_data.blur[0])["=="](3)
	T(shape_data.blur[1])["=="](5)
...
T.Test2D("Graphics render2d SetBlendPreset and GetBlendMode", function()
	local function assert_blend_mode_matches_preset(mode_name)
		local blend_mode = render2d.GetBlendMode()
		local preset = render2d.blend_modes[mode_name]
		T(type(blend_mode))["=="]("table")
		T(blend_mode.blend)["=="](preset.blend)
		T(blend_mode.src_color_blend_factor)["=="](preset.src_color_blend_factor)
		T(blend_mode.dst_color_blend_factor)["=="](preset.dst_color_blend_factor)
		T(blend_mode.color_blend_op)["=="](preset.color_blend_op)
	...
	assert_blend_mode_matches_preset("additive")
	render2d.SetBlendPreset("multiply")
	assert_blend_mode_matches_preset("multiply")
	render2d.SetBlendPreset("premultiplied")
	assert_blend_mode_matches_preset("premultiplied")
...
T.Test2D("Graphics render2d PushBlendPreset and PopBlendMode", function()
	render2d.PushBlendPreset("alpha")
	render2d.SetBlendPreset("additive")
	T(render2d.GetBlendMode().dst_color_blend_factor)["=="]("one")
	render2d.PopBlendMode()
	T(render2d.GetBlendMode().dst_color_blend_factor)["=="]("one_minus_src_alpha")
end)

T.Test2D("Graphics render2d explicit blend factors roundtrip", function()
	render2d.SetBlendMode("src_alpha", "one", "add", "one", "one", "add")
	local blend_mode = render2d.GetBlendMode()
	T(type(blend_mode))["=="]("table")
	T(blend_mode.src_color_blend_factor)["=="]("src_alpha")
	T(blend_mode.dst_color_blend_factor)["=="]("one")
	T(blend_mode.color_blend_op)["=="]("add")
	T(blend_mode.src_alpha_blend_factor)["=="]("one")
	T(blend_mode.dst_alpha_blend_factor)["=="]("one")
	T(blend_mode.alpha_blend_op)["=="]("add")
	render2d.PushBlendPreset("alpha")
	T(render2d.GetBlendMode().dst_color_blend_factor)["=="]("one_minus_src_alpha")
	render2d.PopBlendMode()
...
T.Test2D("Graphics render2d invalid blend preset", function()
	local success, err = pcall(function()
		render2d.SetBlendPreset("invalid_mode")
	end)
	T(success)["=="](false)
...
T.Test2D("Graphics render2d CreateMesh attribute API", function()
	local mesh = render2d.CreateMesh(3, {0, 1, 2})
	mesh:SetVertex(1, "pos", 10, 20)
	mesh:SetVertex(1, "uv", 0.25, 0.75)
	mesh:SetVertex(1, "color", 1, 0.5, 0.25, 0.75)
	local x, y, z = mesh:GetVertex(1, "pos")
	local u, v = mesh:GetVertex(1, "uv")
	local r, g, b, a = mesh:GetVertex(1, "color")
	T(x)["=="](10)
	T(y)["=="](20)
	T(z)["=="](0)
	T(u)["=="](0.25)
...
T.Test2DFrames(
	...
	function(width, height, frame)
		if frame == 1 then
			...
			T.AssertScreenPixel{pos = {84, 44}, color = {1, 0, 0, 1}, tolerance = 0.1}
		else
			T.AssertScreenPixel{pos = {44, 84}, color = {1, 1, 0, 1}, tolerance = 0.1}
			T.AssertScreenPixel{pos = {84, 44}, color = {0, 0, 0, 1}, tolerance = 0.1}
		end
...
T.Test2DFrames(
	...
	function(width, height, frame)
		if frame == 1 then
			...
			T.AssertScreenPixel{pos = {64, 64}, color = {0, 1, 0, 1}, tolerance = 0.1}
		end
	end
)
