...
test("Ang3 GetForward", function()
	-- Looking down the x-axis (yaw=0, pitch=0)
	local a = Ang3(0, 0, 0)
	local fwd = a:GetForward()
	ok(math.abs(fwd.x - 1) < 0.0001, "forward x should be ~1")
	ok(math.abs(fwd.y) < 0.0001, "forward y should be ~0")
	ok(math.abs(fwd.z) < 0.0001, "forward z should be ~0")
end)

test("Ang3 GetForward with yaw", function()
	-- Looking along y-axis (yaw=pi/2)
	local a = Ang3(0, math.pi / 2, 0)
	local fwd = a:GetForward()
	ok(math.abs(fwd.x) < 0.0001, "forward x should be ~0")
	ok(math.abs(fwd.y - 1) < 0.0001, "forward y should be ~1")
	ok(math.abs(fwd.z) < 0.0001, "forward z should be ~0")
end)

test("Ang3 GetUp", function()
	local a = Ang3(0, 0, 0)
	local up = a:GetUp()
	ok(math.abs(up.z - 1) < 0.0001, "up z should be ~1")
end)
