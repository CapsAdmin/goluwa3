...
local function drawArrow(x1, y1, x2, y2, size)
	...
	render2d.DrawLine(x1, y1, x2, y2, 2)
	render2d.DrawLine(x2, y2, x2 - size * math.cos(angle - 0.5), y2 - size * math.sin(angle - 0.5), 2)
	render2d.DrawLine(x2, y2, x2 - size * math.cos(angle + 0.5), y2 - size * math.sin(angle + 0.5), 2)
end
...
event.AddListener("Draw2D", "fonts_metrics_diagram", function()
	...
	render2d.DrawLine(ox + xMax, vyMax, ox + xMax, vyMax - 60, 1)
	labelFont:DrawText("xMax", ox + xMax - 15, vyMax - 65)
	render2d.DrawLine(ox + xMax, vyMax, ox + xMax + 60, vyMax, 1)
	labelFont:DrawText("yMax", ox + xMax + 65, vyMax - 5)
	render2d.DrawLine(ox + xMax, vyMin, ox + xMax + 60, vyMin, 1)
