...
function ProceduralTerrainHybridRenderer:CreateTileTextures(bounds, config)
	...
	height_texture:Shade(
		self.Source:BuildHeightShader(bounds.min_x, bounds.min_z, world_size_x, height_size, height_size),
		{header = self.Source:GetShaderHeader()}
	)
	...
	albedo_texture:Shade(
		self.Source:BuildAlbedoShader(bounds.min_x, bounds.min_z, world_size_x, albedo_size, albedo_size),
	...
	material_texture:Shade(
		self.Source:BuildMaterialShader(bounds.min_x, bounds.min_z, world_size_x, material_size, material_size),
		{header = self.Source:GetMaterialShaderHeader()}
	)
...
function ProceduralTerrainHybridRenderer:RemoveRenderCache()
	for _, render_data in pairs(self.TileRenderCache) do
		if
